...
const mapToGuns = (items, favoritesItems) => {
...
class GunController {
    async createGun(req, res) {
        try {
            ...
            await Promise.all([
                rootDocumentForGuns.addToItems(gun),
                Category.updateMany(
    ...
    async getGuns(req, res) {
        ...
        try {
            if (categories && !sort) {
                ...
                const guns = mapToGuns(items, req.favorites.items);
            ...
            if (sort && !categories) {
                ...
                if (type.toLowerCase() === 'price' && Math.abs(+param) === 1) {
                    const promiseGuns = await Gun.find()
            ...
            const guns = mapToGuns(rootDocumentForGuns.items, req.favorites.items);
            res.status(200).json({
                succes: true,
                items: guns,
